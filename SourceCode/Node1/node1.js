...
function OnrecievingAppendEntryRPC() {
    checkHeartBeat = setInterval(function () {
        //dummy condition
        Received += 1;
        //for second time election
        if (currentRole == 'Follower' && timeout === true && currentLeader !== null && startElection === false && Received > ReceivedLeaderCount && ReceivedVoteREquest === false) {
            ...
            console.log('First election node1');
            vote = setTimeout(() => {
...
function RequestVoteRPC() {
    ...
    if (followerlogInfo?.length > 0) {
        ...
        var allServer = ['Node2', 'Node3', 'Node4', 'Node5'];
        // Edit later
...
function votingLeader(response = {}, info = {}) {
    // Need to update according to the new logic
    votedFor = null;
    if (response?.term > currentTerm && currentRole != "leader" && response.term > lastTermvotedfor) {
        currentRole = "Follower";
...
function StoreClientInfo(response = {}) {
    ...
    if(currentRole === 'leader') {
        ...
        console.log('store request received: at leader', newStoreObject);
        ...
        console.log('store request received: at follower', StoreRejectMsg);
...
function RetrieveRequestInfo(response = {}) {
    console.log('retrieve request received:  node1', RetrieveRejectMsg);
    if(currentRole === 'leader') {
        ...
        console.log('retrieve request received: leader node1', RetrieveRejectMsg);
        server.send(BufferMsg, 5555, response?.sender_name, function (error) {
            if (error) {
                server.close();
                console.log(error)
        ...
        console.log('retrieve request received: follower node1', RetrieveRejectMsg);
